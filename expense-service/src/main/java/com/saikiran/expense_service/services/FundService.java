...
public class FundService {
    ...
    public Long getOrCreateSelfFundId(String userId) {
        return fundRepository.findFundIdByUserIdAndOwnerTypeForUpdate(userId, "SELF")
                .orElseGet(() -> {
    ...
    public Long getOtherFundInfo(String userId, String ownerName) {
        ...
        FundInfo fund = fundRepository.findByUserIdAndOwnerTypeAndOwnerName(userId, "OTHER", ownerName)
                .orElseThrow(() -> new FundNotFoundException("Fund not found for owner: " + ownerName));

        return fund.getFundId();
    }

    @Transactional
    public FundResponse addFund(CreateFundRequest dto) {
        // SELF fund
        if ("SELF".equals(dto.getOwnerType())) {
            dto.setOwnerName(null);

            // CHECK EXISTING SELF
            Optional<FundInfo> existingSelf = fundRepository.findByUserIdAndOwnerType(dto.getUserId(), "SELF");

            // RETURN EXISTING
            if (existingSelf.isPresent()) {
                return fundMapper.toFundResponse(existingSelf.get());
            }
        }

        if (fundRepository.existsByUserIdAndOwnerTypeAndOwnerName(dto.getUserId(), dto.getOwnerType(), dto.getOwnerName())) {
            throw new DuplicateFundException("Fund already exists");
        }

        // OTHER validation
        if ("OTHER".equals(dto.getOwnerType()) && dto.getOwnerName() == null) {
            throw new IllegalArgumentException("Owner name is required for OTHER fund");
        }

        FundInfo fundInfo = fundMapper.toFundInfo(dto);

        // initialize remainingAmount
        fundInfo.setRemainingAmount(fundInfo.getAmountReceived());
        fundInfo.setStatus(FundStatus.ACTIVE);
        fundInfo.setCreatedDate(LocalDate.now());
    ...
    public List<FundResponse> getFundList(String userId) {
        ...
        return fundInfoList.stream()
                .filter(fund -> !"SELF".equalsIgnoreCase(fund.getOwnerType()))
    ...
    public List<FundUtilizationResponse> getFundUtilization(String userId) {
        List<FundInfo> funds = fundRepository.findByUserId(userId)
                .stream()
                .filter(fund -> !"SELF".equals(fund.getOwnerType()))
                .toList();

        return funds.stream()
                .map(fund -> {
                    BigDecimal used = fund.getAmountReceived().subtract(fund.getRemainingAmount());
                    Integer percentage = fund.getAmountReceived().compareTo(BigDecimal.ZERO) == 0
                            ? 0
                            : used.multiply(BigDecimal.valueOf(100))
                                  .divide(fund.getAmountReceived(), 0, RoundingMode.HALF_UP)
                                  .intValue();

                    return FundUtilizationResponse.builder()
                            .ownerName(fund.getOwnerName())
                            .received(fund.getAmountReceived())
                            .remaining(fund.getRemainingAmount())
                            .used(used)
