...
public class ExpenseController {
    ...
    public ResponseEntity<ExpenseResponse> addExpense(
        ...
        return ResponseEntity.ok(expenseService.addExpense(request, key));
    }
    ...
    public ResponseEntity<PaginatedResponse<ExpenseResponse>> getExpenses(
        ...
        return ResponseEntity.ok(expenseService.getExpenses(userId, page, size, sortBy));
    }

    @GetMapping("/expenses/{expenseId}")
    public ResponseEntity<ExpenseResponse> getExpensesByExpenseId(
            @RequestHeader("x-user-id") String userId,
            @PathVariable Long expenseId
    ) {
        return ResponseEntity.ok(expenseService.getExpenseByExpenseId(userId, expenseId));
    }
    ...
    public ResponseEntity<?> getCategoryTotals(@RequestHeader("x-user-id") @NonNull String userId) {
        return ResponseEntity.ok(expenseService.getCategoriesExpense(userId));
    }

    @GetMapping("/fund/{fundId}")
    public ResponseEntity<List<ExpenseResponse>> getExpensesByFundId(
            @RequestHeader("x-user-id") @NonNull String userId,
    ...
    public ResponseEntity<List<MonthlyAnalyticsResponse>> getMonthlyAnalytics(@RequestHeader("x-user-id") String userId) {
        return ResponseEntity.ok(expenseService.getMonthlyAnalytics(userId));
    }

    @GetMapping("/analytics/weekly")
    public ResponseEntity<List<WeeklyTrendResponse>> getWeeklyTrend(@RequestHeader("x-user-id") String userId) {
        return ResponseEntity.ok(expenseService.getWeeklyTrend(userId));
    }

    @GetMapping("/analytics/top-category")
    public ResponseEntity<Map.Entry<String, BigDecimal>> getTopCategory(@RequestHeader("x-user-id") String userId) {
    ...
    public ResponseEntity<List<ExpenseResponse>> getExpensesByDateRange(
        ...
        return ResponseEntity.ok(customDateService.getExpensesByRange(userId, range, startDate, endDate));
    }
